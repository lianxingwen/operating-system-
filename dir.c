...
#include <string.h>
#include "FILESYS.H"
void _dir() /* _dir */ // 显示当前目录下的所有条目，包括文件和子目录的权限、大小及数据块信息
{
...
void mkdir(char *dirname) /* mkdir新建一个目录 */
{
	int dirid, dirpos;
	...
	// 初始化新目录数据块（含"."和".."）
	strcpy(buf[0].d_name, ".");  // strcpy (buf[0].d_name,".");
	buf[0].d_ino = dirid;								  // 第一个指向自己
	strcpy(buf[1].d_name, ".."); // strcpy(buf[1].d_name,"..");
	buf[1].d_ino = cur_path_inode->i_ino;				  // 第二个指向父目录
	block = balloc();									  // 分配数据块

	if (block == DISKFULL)
	{
		ifree(inode->i_ino); // 释放已分配的inode
...
void chdir(char *dirname) /* chdir目录切换 */
{
	unsigned int dirid;
	struct inode *inode;
